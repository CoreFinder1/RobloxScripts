...
Abilities:NewButton("Silent/Heal Build", "Changes Perk Loadout", function()
local args = {
	"Loadout",
	"Perk",
	"Survivor",
...
UserInputService.InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.Two then
...
UserInputService.InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.Four then
...
UserInputService.InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.Eight then
    local args = {
    [1] = "Died"
}
